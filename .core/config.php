<?php
require_once(str_replace('\\\\','/',dirname(__FILE__)).'/../../.core/consts.php');
...
SpawConfig::setStaticConfigItem('default_stylesheet','/themes/'.THEME_NAME.'/site.css');
// width
...
//FERN==============================================
	$dirs = array();
	$skipdirs = array("/.spaw", "/admin", "/.js", "/.core", "/.errordocs");
	function spaw_recursedir($dirname){
		global $dirs, $skipdirs;
		$dir = opendir("{$_SERVER["DOCUMENT_ROOT"]}/$dirname");
		while ($fname = readdir($dir)) {
			if ( in_array($fname, array(".", "..", ".svn", "prop-base", "props", "text-base", "tmp")) ){
				continue;
			}
	
			if (is_dir("{$_SERVER["DOCUMENT_ROOT"]}/$dirname/$fname") && !in_array("$dirname/$fname", $skipdirs) ){
				$dirs[] = "$dirname/$fname";
				spaw_recursedir("$dirname/$fname");
			}
		}
		closedir($dir);
		return $dirs;
	}
	//--- skip reading of dir to speed up script ---//
	if(LIVEMODE == "yes" ){
		//--- almost useless but very heavy ---//
		$dirs = spaw_recursedir("");	 
		sort($dirs);
		print_r($dirs);
		exit();
...
/*			
	...
	*/
		foreach ($dirs as $dir){
			...
			if ($dir=="/files/images")
				$diritem["params"]["default_dir"] = true;
			$spaw_imglibs[] = $diritem;
		}
	}else{
		$spaw_imglibs = array();
		$spaw_imglibs[] = array ( "dir" => "files/", "caption" => "/files" );
...
SpawConfig::setStaticConfigItem(
  ...
  array(
    ...
    array(
      'dir'     => SpawConfig::getStaticConfigValue('SPAW_DIR').'files/images/',
      'caption' => 'Images',
      'params'  => array(
    ...
    array(
      'dir'     => SpawConfig::getStaticConfigValue('SPAW_DIR').'files/',
      'fsdir'   => SpawConfig::getStaticConfigValue('DOCUMENT_ROOT').'files/', // optional absolute physical filesystem path
      'caption' => 'Files',
      'params'  => array(
